...
export default function ProductCard({ id, name, size, img, height, type, pricePerBox }: Props) {
  ...
  return (
    <>
      <motion.div
        ...
        onClick={() => setSelectedProduct({ id, name, size, img, height, type, pricePerBox })}
      >
        <div className="relative flex items-end justify-center w-full min-h-[260px] sm:min-h-[320px]">
          <div className="absolute inset-0 bg-[#0066FF]/20 blur-3xl opacity-0 group-hover:opacity-100 transition duration-700 rounded-full" />
          <motion.div whileHover={{ scale: 1.1, y: -10 }} className="relative z-10">
            <Image src={img} alt={name} width={220} height={300}
        ...
        <div className="mt-5 text-center">
          ...
          <div className="mt-2">
            <p className="text-sm font-bold text-green-600">₹{pricePerBox.toLocaleString("en-IN")}/box</p>
            <p className="text-xs text-gray-400">(Min. 50 boxes)</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-5 w-full">
          <button
            ...
            className={`flex-1 px-3 py-2 rounded-xl text-xs flex items-center justify-center gap-2 border backdrop-blur-md transition-all duration-300 ${
              added ? "bg-green-500 text-white border-green-400 shadow-green-300/40"
        ...
        {selectedProduct && (
          ...
          >
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }}
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <Image src={selectedProduct.img} alt={selectedProduct.name} width={250} height={250}
              ...
              <p className="text-lg font-bold text-green-600 mt-1">
                ₹{selectedProduct.pricePerBox.toLocaleString("en-IN")}/box
              </p>
              <div className="mt-5 flex gap-3 w-full">
                <button onClick={handleAddToCart}
                  className="flex-1 py-2 rounded-xl bg-[#0066FF] text-white shadow-lg hover:bg-blue-600 transition">Add</button>
                <button onClick={handleBuyNow}
                  className="flex-1 py-2 rounded-xl bg-gradient-to-r from-[#0066FF] to-blue-500 text-white shadow-lg hover:scale-105 transition">Buy</button>
              </div>
            </motion.div>
          </motion.div>
        )}
