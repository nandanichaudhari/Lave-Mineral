    ...
    fetch("/api/pricing")
      ...
      .then((data) => {
        if (data.success) {
          ...
          data.prices.forEach((p: { productId: number; pricePerBox: number }) => {
            priceMap[p.productId] = p.pricePerBox;
          });
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-white via-[#f0f6ff] to-[#e6f0ff] text-slate-900 font-sans overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none -z-10">
        <div className="absolute w-[400px] h-[400px] bg-[#0066FF]/20 blur-3xl rounded-full top-10 left-10 animate-pulse" />
        <div className="absolute w-[350px] h-[350px] bg-blue-400/20 blur-3xl rounded-full bottom-10 right-10 animate-pulse" />
      </div>
      ...
      <header className="pt-24 pb-16 text-center px-4">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <div className="h-[2px] w-20 bg-gradient-to-r from-[#0066FF] to-blue-400 mx-auto mb-5 rounded-full" />
          <p className="text-[10px] sm:text-xs tracking-[0.5em] text-slate-400 uppercase">
            Admin View • Prices reflect live DB values
          </p>
        </motion.div>
      </header>
        ...
        <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-20">
          {filteredProducts.map((product, i) => (
            ...
            >
              <ProductCard
                {...product}
                pricePerBox={livePrices[product.id] ?? product.pricePerBox}
              />
            </motion.div>
          ))}
        </motion.div>
